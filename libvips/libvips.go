...
#cgo LDFLAGS: -L../lib -Wl,-rpath,\$ORIGIN/lib -lvips.dll -lglib-2.0.dll -lgobject-2.0.dll
#include <stdlib.h>
#include <stdio.h>
#include "vips/vips.h"
#include "libvips.h"
...
func Startup(config *Config) {
	if hasShutdown {
		panic("golibvips cannot be stopped and restarted")
	}
	...
	if MajorVersion < 8 {
		panic("golibvips requires libvips version 8.10+")
	}
	...
	if err != 0 {
		panic(fmt.Sprintf("Failed to start libvips code=%v", err))
	}
...
// called when a thread using libvips exits.
func ShutdownThread() {
